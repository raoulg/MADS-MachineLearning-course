...
def init_weights(
    ...
    return scale * torch.normal(0., scale, (m, n)), scale * torch.normal(0., scale, (n,))


def torch_network(sizes: List[int], scale: float = 1e-1) -> List[Tuple[torch.Tensor, torch.Tensor]]:
